...
pub fn search_manga_with_urls_browser(
    ...
    // Navigate to Webtoon Canvas list
    let list_url = format!("{}/en/canvas/list?sortOrder=READ_COUNT", BASE_URL);
    ...
    let _ = std::fs::write("webtoon_series_list.html", &html);
...
pub fn get_chapters_browser(
    ...
    let _ = std::fs::write(format!("webtoon_{}.html", slug), &html);
