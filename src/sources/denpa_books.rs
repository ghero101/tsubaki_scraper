#![allow(dead_code)]
/// Denpa Books - Free content where available
use reqwest::Client;
use crate::models::{Manga, Chapter};
use scraper::{Html, Selector};

const BASE_URL: &str = "https://denpa.pub";
...
pub async fn search_manga_with_urls(
    ...
    let search_url = if title.is_empty() {
        format!("{}/books/", BASE_URL)
    } else {
        format!("{}{}?search={}", BASE_URL, "/books/", urlencoding::encode(title))
    };
    ...
    let selectors = vec![
        "div.series-card a[href*=\'/books/\']",
        "article.series a[href*=\'/books/\']",
        "div.manga-item a[href*=\'/books/\']",
        "a.series-link[href*=\'/books/\']",
        "a[href*=\'/books/\']",
    ];
