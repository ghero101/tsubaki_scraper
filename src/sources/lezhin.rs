...
const BASE_URL: &str = "https://www.lezhinus.com";
...
pub async fn search_manga_with_urls(
    ...
    let search_url = if title.is_empty() {
        format!("{}/comic/", BASE_URL)
    } else {
        format!("{}{}?search={}", BASE_URL, "/comic/", urlencoding::encode(title))
    };
    ...
    let selectors = vec![
        "div.series-card a[href*=\'/comic/\']",
        "article.series a[href*=\'/comic/\']",
        "div.manga-item a[href*=\'/comic/\']",
        "a.series-link[href*=\'/comic/\']",
        "a[href*=\'/comic/\']",
    ];
    ...
    for selector_str in selectors {
        if let Ok(selector) = Selector::parse(selector_str) {
            for element in document.select(&selector).take(10) {
                ...
                if !url.is_empty() && !title_text.is_empty() && url.contains("/comic/") {
                    results.push((Manga {
                        id: String::new(),
                        title: title_text,
                        alt_titles: None,
