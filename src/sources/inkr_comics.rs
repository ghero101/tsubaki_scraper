#![allow(dead_code)]
/// Inkr Comics - Free content where available
use reqwest::Client;
use crate::models::{Manga, Chapter};
use scraper::{Html, Selector};

const BASE_URL: &str = "https://comics.inkr.com";
...
pub async fn search_manga_with_urls(
    ...
    let search_url = if title.is_empty() {
        ...
        format!("{}{}?search={}", BASE_URL, "/title/", urlencoding::encode(title))
    };
    ...
    let selectors = vec![
        "div.series-card a[href*=\'/title/\']",
        "article.series a[href*=\'/title/\']",
        "div.manga-item a[href*=\'/title/\']",
        "a.series-link[href*=\'/title/\']",
        "a[href*=\'/title/\']",
    ];
    ...
    for selector_str in selectors {
        if let Ok(selector) = Selector::parse(selector_str) {
            for element in document.select(&selector).take(10) {
                ...
                if !url.is_empty() && !title_text.is_empty() && url.contains("/title/") {
                    results.push((Manga {
                        id: String::new(),
                        title: title_text,
                        alt_titles: None,
