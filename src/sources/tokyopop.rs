/// Tokyopop - Free content where available
use reqwest::Client;
use crate::models::{Manga, Chapter};
use scraper::{Html, Selector};

const BASE_URL: &str = "https://www.tokyopop.com";
...
pub async fn search_manga_with_urls(
    ...
    let search_url = if title.is_empty() {
        format!("{}/product/", BASE_URL)
    } else {
        format!("{}{}?search={}", BASE_URL, "/product/", urlencoding::encode(title))
    };
    ...
    let selectors = vec![
        "div.series-card a[href*=\'/product/\']",
        "article.series a[href*=\'/product/\']",
        "div.manga-item a[href*=\'/product/\']",
        "a.series-link[href*=\'/product/\']",
        "a[href*=\'/product/\']",
    ];
    ...
    for selector_str in selectors {
        if let Ok(selector) = Selector::parse(selector_str) {
            for element in document.select(&selector).take(10) {
                ...
                if !url.is_empty() && !title_text.is_empty() && url.contains("/product/") {
                    results.push((Manga {
                        id: String::new(),
                        title: title_text,
                        alt_titles: None,
