        ...
        for (container_sel, link_sel) in &selectors {
            if let Ok(container_selector) = Selector::parse(container_sel) {
                for element in document.select(&container_selector) {
                    if let (Ok(_link_selector), Some(title_element)) = (
                        Selector::parse(link_sel),
                        element.select(&Selector::parse(link_sel).unwrap()).next(),
                    ) {
                        let title = title_element.text().collect::<String>().trim().to_string();
                        let series_url =
                            title_element.value().attr("href").unwrap_or("").to_string();
        ...
        loop {
            ...
            for element in document.select(&selector) {
                if let Some(title_element) =
                    element.select(&Selector::parse("h3 > a").unwrap()).next()
                {
                    let title = title_element.text().collect::<String>().trim().to_string();
    ...
    if chapters.is_empty() {
        ...
        for n in 2..=10u32 {
            ...
            for u in urls {
                ...
                if let Ok(selector) =
                    Selector::parse("a.chapter, ul.chapter-list li a, li.wp-manga-chapter a")
                {
