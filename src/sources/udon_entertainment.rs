#![allow(dead_code)]
/// Udon Entertainment - Free content where available
use reqwest::Client;
use crate::models::{Manga, Chapter};
use scraper::{Html, Selector};

const BASE_URL: &str = "https://www.udonentertainment.com";
...
pub async fn search_manga_with_urls(
    ...
    let search_url = if title.is_empty() {
        ...
        format!("{}{}?search={}", BASE_URL, "/products/", urlencoding::encode(title))
    };
    ...
    let selectors = vec![
        "div.series-card a[href*=\'/products/\']",
        "article.series a[href*=\'/products/\']",
        "div.manga-item a[href*=\'/products/\']",
        "a.series-link[href*=\'/products/\']",
        "a[href*=\'/products/\']",
    ];
    ...
    for selector_str in selectors {
        if let Ok(selector) = Selector::parse(selector_str) {
            for element in document.select(&selector).take(10) {
                ...
                if !url.is_empty() && !title_text.is_empty() && url.contains("/products/") {
                    results.push((Manga {
                        id: String::new(),
                        title: title_text,
                        alt_titles: None,
