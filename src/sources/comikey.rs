#![allow(dead_code)]
/// Comikey - Free content where available
use reqwest::Client;
...
pub async fn search_manga_with_urls(
    ...
    let search_url = if title.is_empty() {
        ...
        format!("{}{}?search={}", BASE_URL, "/comics/", urlencoding::encode(title))
    };
    ...
    let selectors = vec![
        "div.series-card a[href*=\'/comics/\']",
        "article.series a[href*=\'/comics/\']",
        "div.manga-item a[href*=\'/comics/\']",
        "a.series-link[href*=\'/comics/\']",
        "a[href*=\'/comics/\']",
    ];
    ...
    for selector_str in selectors {
        if let Ok(selector) = Selector::parse(selector_str) {
            for element in document.select(&selector).take(10) {
                ...
                if !url.is_empty() && !title_text.is_empty() && url.contains("/comics/") {
                    results.push((Manga {
                        id: String::new(),
                        title: title_text,
                        alt_titles: None,
