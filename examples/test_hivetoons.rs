use reqwest::Client;
use rust_manga_scraper::sources::hivetoons;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test manga search
    let results = hivetoons::search_manga_with_urls(&client, "").await?;
    println!("✓ Found {} manga\n", results.len());
    ...
    for (manga, url) in results.iter().take(3) {
        ...
        match hivetoons::get_chapters(&client, &url).await {
            Ok(chapters) => {
    ...
    for (_, url) in results.iter().take(3) {
        if let Ok(chapters) = hivetoons::get_chapters(&client, url).await {
