...
use rust_manga_scraper::sources::kenscans_browser;

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== Testing KenScans with Browser Automation ===\n");
    ...
    let results = kenscans_browser::search_manga_with_urls_browser(&manager, "")?;
    println!("   ✓ Found {} manga\n", results.len());
    ...
    for (manga, url) in results.iter().take(3) {
        ...
        match kenscans_browser::get_chapters_browser(&manager, url) {
            Ok(chapters) => {
    ...
    for (_, url) in results.iter().take(3) {
        if let Ok(chapters) = kenscans_browser::get_chapters_browser(&manager, url) {
