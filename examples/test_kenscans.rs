use reqwest::Client;
use rust_manga_scraper::sources::kenscans;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test manga search
    let results = kenscans::search_manga_with_urls(&client, "").await?;
    println!("✓ Found {} manga\n", results.len());
    ...
    for (manga, url) in results.iter().take(3) {
        ...
        match kenscans::get_chapters(&client, &url).await {
            Ok(chapters) => {
    ...
    for (_, url) in results.iter().take(3) {
        if let Ok(chapters) = kenscans::get_chapters(&client, url).await {
